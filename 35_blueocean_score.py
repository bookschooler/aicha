...
# ─────────────────────────────────────────────
# 2. 공통 분위수 계산
#    residual_pct_asc: 낮은 잔차(음수) = 낮은 값 (Q2에서 1-pct로 변환)
#    supply_pct:       높은 공급부족 = 높은 점수 (두 접근법 공통)
# ─────────────────────────────────────────────
df['residual_pct'] = df['residual_latest'].rank(pct=True, ascending=True)
df['supply_pct']   = df['supply_shortage'].rank(pct=True, ascending=True)
...
save_base = [
    ...
    '찻집수_latest', '매출_latest', '사분면', '구조적블루오션'
]
